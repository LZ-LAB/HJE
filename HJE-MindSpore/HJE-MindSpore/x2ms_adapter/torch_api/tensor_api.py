...
def _get_shape_index(tensor, index):
    ...
    return new_shape, tuple(new_index)
...
_TENSOR_DTYPE_RANGE = {
    str(mindspore.float16): (-65504, 65504), str(mindspore.int16): (-32768, 32767),
...
@x2ms_func_decorator(mindspore.Tensor)
def swapaxes(obj, axis0, axis1):
    return torch_base_api.swapaxes(obj, axis0, axis1)
...
@x2ms_func_decorator(mindspore.Tensor)
def histogram(obj, bins, *, range=None, weight=None, density=False):
    return torch_base_api.histogram(obj, bins, range=range, weight=weight, density=density)


@x2ms_func_decorator(mindspore.Tensor)
def hypot(obj, other):
    return torch_base_api.hypot(obj, other)


@x2ms_func_decorator(mindspore.Tensor)
def inner(obj, other):
    return torch_base_api.inner(obj, other)


@x2ms_func_decorator(mindspore.Tensor)
def lcm(obj, other):
    return torch_base_api.lcm(obj, other)


@x2ms_func_decorator(mindspore.Tensor)
def logaddexp(obj, other):
    return torch_base_api.logaddexp(obj, other)


@x2ms_func_decorator(mindspore.Tensor)
def logaddexp2(obj, other):
    return torch_base_api.logaddexp2(obj, other)


@x2ms_func_decorator(mindspore.Tensor)
def nanmean(obj, dim=None, keepdim=False, *, dtype=None):
    return torch_base_api.nanmean(obj, dim=dim, keepdim=keepdim, dtype=dtype)
