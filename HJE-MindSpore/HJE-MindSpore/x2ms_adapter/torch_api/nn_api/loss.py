...
class NLLLoss(mindspore.nn.LossBase):
    ...
    def construct(self, input, target):
        if input.ndim < 2 or input.ndim > 4:
        ...
        if input.ndim == 3:
            ...
            _target = self.one_hot(target, input.shape[1], self.on_value, self.off_value).transpose(0, 3, 1, 2)
        if self.reduction == "sum":
...
class NLLLoss2d(NLLLoss):
    ...
    def construct(self, input, target):
        return super(NLLLoss2d, self).construct(input, target)
