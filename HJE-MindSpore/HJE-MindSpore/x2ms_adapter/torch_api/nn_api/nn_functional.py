...
def x2ms_pad(input, pad, mode='constant', value=0):
    ...
    return ops.Pad(tuple(new_pad))(input)
...
def upsample(input, size=None, scale_factor=None, mode='nearest', align_corners=None):
    ...
    """
    if mode in ('linear', 'bicubic', 'trilinear'):
        raise NotImplementedError(f"{mode} is not supported in mindspore yet")
    if isinstance(scale_factor, tuple):
        raise NotImplementedError("upsample in MindSpore doesn't support scale_factor to be tuple.")
    if isinstance(size, int):
        raise NotImplementedError("upsample in MindSpore doesn't support size to be int.")
    ...
    if scale_factor is not None:
        size = list(sub_size * scale_factor for sub_size in input_size[input.ndim - 2:])
    if mode == 'nearest':
        upsample_result = mindspore.ops.ResizeNearestNeighbor(size, align_corners=align_corners)
    else:
        upsample_result = mindspore.ops.ResizeBilinear(size, align_corners=align_corners)
    return upsample_result(input)
